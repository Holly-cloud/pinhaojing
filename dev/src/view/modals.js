...
/* ==================== v7.21 · 项目总览面板（需求三） ====================
   ...
   v7.21b 低门槛改版（用户实测反馈：旧版「可读性极差、门槛过高」）——四项口径：
     · 形态：**宽面板两栏**（左栏＝按用途分组的白话功能清单，右栏＝选中项详情）；宽度变体走 `.map-win`。
     · 技术信息：**只在右栏出现**（右栏即天然的「展开详情」位，不再叠一层折叠控件）。
     · 「代码完善度」：三档**人话状态**（已上线·已验收 / 已上线·待验收 / 规划中）+ 彩色点，不出现公式。
     · 组织方式：**按用途分组**（group）。
   ...
   形态：复用既有窗骨架 CSS 类（.modal-mask/.modal/.modal-title/.modal-body），仅加 `.map-win` 宽度变体；
...
var MAP_STATE_TEXT = { done: '已上线 · 已验收', pending: '已上线 · 待验收', plan: '规划中' };

/* MAP_FEATURES —— 本面板的唯一数据源。
   · group：用途分组名（写作 / 画布 / 项目与界面）
   · name ：**白话**功能名（读者是人，不是开发者）
   · desc ：一句话说明「这功能是干什么的」（人类语言）
   · ver / where / cover：技术证据层（版本 / 代码位置 / 验收证据），只在右栏显示
   · state：done=已上线·已验收 / pending=已上线·待验收 / plan=规划中 */
var MAP_FEATURES = [
  { group: '写作', name: '写作台：大纲 + 编辑器', desc: '把提示词按顺序排成一条大纲，逐条编辑',
    ver: 'v7.15', where: 'view/write.js · editor/host.js', cover: 'W 组 21 条', state: 'done' },
  { group: '写作', name: '写作补全：输入 # 唤起词库', desc: '打了 # 就从你看过的内容里补全',
    ver: 'v7.8–v7.16', where: 'editor/complete.js · editor/library.js', cover: 'C 组 31 条', state: 'done' },
  { group: '写作', name: '块标签「初 / 补」+ 自动分组', desc: '标出这条是初稿还是补充，同类的自动归到一起',
    ver: 'v7.21', where: 'view/write.js · core/persist.js', cover: 'v7.21 组 6 条', state: 'done' },
  { group: '写作', name: '放大编辑：逗号自动转空格', desc: '双击单条放大精修',
    ver: 'v7.17', where: 'editor/block-editor.js', cover: 'E 组 16 条', state: 'done' },
  { group: '写作', name: '编辑器粘贴不被画布抢走', desc: '在编辑器里粘贴，内容不会跑到画布上',
    ver: 'v7.18', where: 'interact/paste.js', cover: 'P 组 4 条', state: 'done' },
  { group: '画布', name: '画布排布与拖拽', desc: '自由摆放、拖动、缩放提示词块',
    ver: 'v6–v7.20', where: 'view/canvas.js · interact/pointer.js', cover: '4 个套件共 171 条', state: 'done' },
  { group: '画布', name: '划选与重影修复', desc: '划选不出重影，补全上屏正确写回',
    ver: 'v7.19', where: 'editor/highlight.js · editor/complete.js', cover: 'v7.19 组 10 条', state: 'done' },
...
/* 三档状态 → 人话文案（未知值兜底为「规划中」，避免将来新增状态时渲染空白） */
function mapStateText(s){
  return MAP_STATE_TEXT[s] ? MAP_STATE_TEXT[s] : MAP_STATE_TEXT.plan;
}
/* 状态点：done=绿 / pending=琥珀 / plan=灰（未知值兜底为 plan 灰） */
function mapDot(state){
  var s = (state === 'done' || state === 'pending') ? state : 'plan';
  var d = document.createElement('span');
  d.className = 'map-dot ' + s;
  return d;
}
/* 顶部：健康度条（一色点 + 一句话） + 三个 metric（功能 / 验收道 / 自动检查） */
function mapBuildTop(){
  var top = document.createElement('div');
  top.className = 'map-top';

  var doneN = 0, pendingN = 0, planN = 0;
  for(var i = 0; i < MAP_FEATURES.length; i++){
    var s = MAP_FEATURES[i].state;
    if(s === 'pending') pendingN++;
    else if(s === 'plan') planN++;
    else doneN++;
  }
  var health = document.createElement('div');
  health.className = 'map-health';
  health.appendChild(mapDot(pendingN > 0 ? 'pending' : (planN > 0 ? 'plan' : 'done')));
  var ht = document.createElement('span');
  /* ★数字一律动态取（项数 = MAP_FEATURES.length、道数 = MAP_GATE_TOTAL）——面板不自带"写死即过期"的数字；
     句末 `.map-stamp`「（交付时）」显式标注时效：这是**交付时快照**，不是实时探测（实时结果以 run-gate 为准）。 */
  if(pendingN === 0 && planN === 0){
    ht.textContent = MAP_FEATURES.length + ' 项功能全部已上线 · ' + MAP_GATE_TOTAL + ' 道自动验收全部通过';
  } else {
    ht.textContent = '已上线 ' + doneN + ' 项，待验收 ' + pendingN + ' 项，规划中 ' + planN + ' 项';
  }
  health.appendChild(ht);
  var stamp = document.createElement('span');
  stamp.className = 'map-stamp';
  stamp.textContent = '（交付时）';
  health.appendChild(stamp);
  top.appendChild(health);

  var kv = document.createElement('div');
  kv.className = 'map-kv';
  var metrics = [
    { n: MAP_FEATURES.length, lab: '功能' },
    { n: MAP_GATE_TOTAL, lab: '验收道' },
    { n: MAP_ASSERT_TOTAL, lab: '自动检查' }
  ];
  for(var j = 0; j < metrics.length; j++){
    var m = document.createElement('div');
    m.className = 'map-metric';
    var mn = document.createElement('div');
    mn.className = 'map-metric-n';
    mn.textContent = String(metrics[j].n);
    var ml = document.createElement('div');
    ml.className = 'map-metric-lab';
    ml.textContent = metrics[j].lab;
    m.appendChild(mn); m.appendChild(ml);
    kv.appendChild(m);
  }
  top.appendChild(kv);
  return top;
}
/* 左栏：按用途分组的功能清单（分组标题 + 白话功能名 + 状态点）；可滚动 */
function mapBuildList(){
...
/* 右栏技术证据的一行（标签 + 值） */
function mapTechRow(k, v){
  var row = document.createElement('div');
  row.className = 'map-tech-row';
  var ks = document.createElement('span');
  ks.className = 'map-tech-k';
  ks.textContent = k;
  var vs = document.createElement('span');
  vs.className = 'map-tech-v';
  vs.textContent = v;
  row.appendChild(ks); row.appendChild(vs);
  return row;
}
/* 右栏详情：白话说明在前，技术证据（代码位置 / 验收证据）在后 */
function mapDetailNode(f){
  var d = document.createElement('div');
  d.className = 'map-detail';
  var dn = document.createElement('div');
  dn.className = 'map-dname';
  dn.textContent = f.name;
  var ds = document.createElement('div');
  ds.className = 'map-desc';
  ds.textContent = f.desc;
  var badges = document.createElement('div');
  badges.className = 'map-badges';
  var st = document.createElement('span');
  st.className = 'map-state ' + ((f.state === 'done' || f.state === 'pending') ? f.state : 'plan');
  st.textContent = mapStateText(f.state);
  var vv = document.createElement('span');
  vv.className = 'map-ver';
  vv.textContent = f.ver;
  badges.appendChild(st); badges.appendChild(vv);
  var tech = document.createElement('div');
  tech.className = 'map-tech';
  tech.appendChild(mapTechRow('代码位置', f.where));
  tech.appendChild(mapTechRow('验收证据', f.cover));
  d.appendChild(dn); d.appendChild(ds); d.appendChild(badges); d.appendChild(tech);
  return d;
}
/* 左→右联动：把选中态移到 idx，并让右栏显示该项详情 */
function mapSelect(listEl, detailEl, idx){
  var rows = listEl.querySelectorAll('.map-row');
  for(var i = 0; i < rows.length; i++){
    if(Number(rows[i].dataset.idx) === idx) rows[i].classList.add('on');
    else rows[i].classList.remove('on');
  }
  detailEl.innerHTML = '';
  detailEl.appendChild(mapDetailNode(MAP_FEATURES[idx]));
}
/* 底部注记（人话）：三档状态口径 + 数据交叉校验 */
function mapBuildNote(){
  var note = document.createElement('div');
  note.className = 'map-note';
  note.textContent = '注：新功能在补上自动检查前会显示为琥珀色「待验收」；本面板数据由仓库的常驻断言交叉校验'
    + '（套件名 / 条数 / 版本对不上即闸门报红），不会悄悄过期。'
    + '本面板为交付时状态；验收的实时结果以 node dev/_qa/run-gate.mjs 为准。';
  return note;
}
function mapBuildDom(){
  var wrap = document.createElement('div');
  wrap.className = 'map-wrap';
  wrap.appendChild(mapBuildTop());

  var cols = document.createElement('div');
  cols.className = 'map-cols';
  var list = mapBuildList();
  var detail = document.createElement('div');
  detail.className = 'map-detail';
  cols.appendChild(list); cols.appendChild(detail);
  wrap.appendChild(cols);
  wrap.appendChild(mapBuildNote());

  /* 默认选中第一项 */
  mapSelect(list, detail, 0);
  /* 左栏联动：**元素级** click + 事件委托（不新增任何 document/window 级监听） */
  list.addEventListener('click', function(e){
    var t = e.target;
    var row = (t && t.closest) ? t.closest('.map-row') : null;
    if(!row || !list.contains(row)) return;
    mapSelect(list, detail, Number(row.dataset.idx));
  });
  return wrap;
}
