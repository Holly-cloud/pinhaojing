...
/* 拼好镜 · 一键验收闸门 runner（自包含 / 零依赖 / 只用 node 内建模块）
   ...
   闸门顺序（与 README「改完必过的闸门」一致）：
     [1] 构建           dev/build.mjs                       期望 355975 B（sha256 d7fa407088f105e8bdb83426d70b9612323c453b9bb9b046e393d46985aec364；v7.21 写作台块标签「初/补」+ 大纲分组排列 + 项目总览面板〔v7.21b 低门槛改版：宽面板两栏 / 白话优先 / 技术证据入右栏 / 健康行数字动态+「（交付时）」时效标注〕；v7.21c 节判定修正：structAt 的 label 改纯行级（无标记行「判断中」）、region 保持段落延续 + 灵感气泡群按「当前行内容」重排〔候选组 = 本节相关组 ∪ 行内命中组（区外命中组拉进并置顶）〕；v7.21c 方案 D：切词前剥离行首「镜头N」序号伪 token）
     ...
     [16] v7.21 组      verify/verify_v721.mjs               期望 12/12（v7.21 新增·写作台块标签「初/补」+ 大纲分组排列 + 项目总览面板；
                                                              A×6 标签/分组/迁移零丢失 + B×5 面板/交叉校验 + B6×1 灵感气泡按行排组；**独立成套**，既有 15 道零改动）
