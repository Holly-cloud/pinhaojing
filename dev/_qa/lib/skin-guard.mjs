/* ============================================================================
   ...
   两条断言口径（由 verify_v78 的 R3-A / R3-B 分别断言）：
     ...
     R3-B **零泄漏**（harvestCorpusStrings → 命中数 == 0）
       皮肤里的**长语料字符串**（字面量内容长度 ≥ 40 字）在非皮肤源码文本里**零命中**——
       语料正文（风格包 5 段 / 硬性要求 / 分组语料）**一个字都不许进引擎**。硬零容忍。
...
/**
 * 计算当前源码树下的皮肤边界命中：R3-A 词元命中 + R3-B 长语料命中。
 * @param {string} [srcRoot] 源码根（默认 dev/src）
 * @returns {{skinFiles,engineFiles,tokens,leakTokens,hits,corpus,corpusLeak}}
 */
export function scanDomainHits(srcRoot = DEFAULT_SRC) {
  ...
  const corpusLeak = corpus.filter((s) => engineRaw.includes(s));           /* R3-B：引擎原文，零容忍 */
  return { skinFiles, engineFiles, tokens, leakTokens, hits: leakTokens.length, corpus, corpusLeak };
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  console.log('R3-B 长语料（≥40 字）' + r.corpus.length + ' 条，非皮肤**原文**命中 =' + r.corpusLeak.length +
    ' → ' + (r.corpusLeak.length === 0 ? 'PASS' : 'FAIL'));
  if (r.corpusLeak.length) console.log('  泄漏语料首段：' + JSON.stringify(r.corpusLeak[0].slice(0, 40)));
