...
> 一条命令复现全部验收：`node dev/_qa/run-gate.mjs` → 期望 **13/13 全绿**（83 / 18 / 16 / **54** / 18 / **21** / **31** / **16** / **4** / **9** / **4**），退出码 0。
>
...
| 分区 | 内容 | 存在理由（删了会怎样） | 状态 |
|---|---|---|---|
| `run-gate.mjs` | ★ 一键闸门入口：自动定位浏览器 → 选空闲端口 → 起 headless → 依次跑 **13 道** → 语义化退出码 | 没有它，13 道验收要人工逐条起浏览器跑 | ★ **在用** |
| `lib/` | 共享模块：`browser-detect.mjs`（浏览器探测/端口）、`test-artifact.mjs`（生成「产物 + 1 行访问器」的测试产物，供各在用套件操纵内部态；产物落 `gen/PHJ_test.html`）、**`skin-guard.mjs`（R3 皮肤边界护栏：抽皮肤词元 / 长语料 / `【…】` 完整标记串，列引擎文件，`DOMAIN_HITS_GOLDEN`——「棘轮 + 零泄漏 + 防真空自检」三件事的公用采集层）** | 没有它，CDP 套件无法在「不改产品」前提下触达内部态，R3 棘轮也无处实现 | ★ **在用** |
| `verify/` | **11 个在用套件**：`verify_build_equivalence.mjs`（等价性，闸门第 2 道）、`verify_v7.mjs`（回归 83）、`verify_v76.mjs`（F 18）、`verify_v77.mjs`（G 16）、`verify_v78.mjs`（H+I+X+P1+P2+P3+R0+R1+**R3+R4** **54**）、`verify_w.mjs`（W **21**）、`verify_c.mjs`（C **31**）、`verify_e.mjs`（E **16**，含 E6a 真鼠标点击 toast 撤销）、`verify_paste.mjs`（P **4**，编辑器内 Ctrl+V 不被画布抢占 + S4 反面对照）、`verify_qa_v718.mjs`（M **9**，多项目容器：解链专项 / 切项目隔离 / 导出导入 / 危险操作 / 间距稳定）、`verify_qa_migrate_visual.mjs`（V **4**，老数据 v16→v17 观感逐像素不变） | 闸门的主体；删了等于没有回归保护 | ★ **在用** |
