/* 等价性 / 构建可复现校验：构建产物 vs 交付快照
   用法（默认 = 当前交付快照 v7.14「构建可复现」语义）：
         node dev/_qa/verify/verify_build_equivalence.mjs
         node dev/_qa/verify/verify_build_equivalence.mjs <产物> <基线>
   默认判定：断言「node dev/build.mjs 的产出」（= 根目录 PHJ.html）与「当前交付快照
         dev/_qa/snapshots/PHJ_v7.18_2026-09-19.html」**逐字节一致（含 banner，不做 stripBanner）**。
   ...
   —— 为什么把 v7.7 的语义换掉（改义理由，务必保留本段）——
      ...
      历史基线 PHJ_v7.7_baseline.html / PHJ_v7.8_20260913.html / PHJ_v7.8.1_20260915.html / PHJ_v7.8.2_20260915.html / PHJ_v7.9_20260916.html / PHJ_v7.12_20260917.html / **PHJ_v7.13_20260917.html** 仍在，**可作 argv 指定基线**复跑（见下）。

   历史比对（保留能力，未删）：把历史基线作为第 3 个参数传入，即走旧语义：
         node dev/_qa/verify/verify_build_equivalence.mjs PHJ.html dev/_qa/snapshots/PHJ_v7.8_20260913.html
...
const OLD  = process.argv[3] || path.join(ROOT, 'dev', '_qa', 'snapshots', 'PHJ_v7.18_2026-09-19.html');
