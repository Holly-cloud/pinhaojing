...
/* 体积：**项目容量限制已解除**（2026-09-16，Holly 指示）——
   ...
   · 体积**仍然实测并打印**（B10/C8 两段日志），供人工评估增长是否合理，只是不再阻断也不告警；
   · 历史沿革（保留作档案）：v7.2 立 131072 B(128KB) → v7.7 实测 155315 B → v7.8 214433 B →
     v7.8.1 222400 B → v7.8.2 222359 B → v7.9 222381 B；曾设「工程临时护栏 229376 B（224KB，告警）」与
     「产品硬上限 262144 B（256KB，P0 阻断）」两条阈值，均**随本次解除作废**。
   · 修复史（保留）：旧 B10a/C8a 曾是 `129444/1024 <= 128` 的恒真式、从不读产物（2026-09-14 修好为真 stat）。 */
...
console.log('── B10. 单文件体积（**容量限制已解除**：仅报告，不断言） ──');
const b10SizeB = fs.statSync(OUT).size;
console.log('    体积实测：' + b10SizeB + ' B（' + OUT.replace(/\\/g, '/').replace(/^.*\//, '') + '，真实 stat）');
console.log('    解除说明（2026-09-16）：原「工程临时护栏 229376 B」与「产品硬上限 262144 B」两条阈值均已作废 → 本项**不再断言**，仅打印供人工评估增长是否合理');
...
console.log('── C8. 体积（**容量限制已解除**：仅报告，不断言） ──');
const c8SizeB = fs.statSync(OUT).size;
console.log('    体积实测：' + c8SizeB + ' B（真实 stat；两条阈值已作废 → 不再断言）');
