...
**当前基线（v7.21c，2026-09-22）**：`PHJ.html` = **355975 B** ｜ sha256 `d7fa407088f105e8bdb83426d70b9612323c453b9bb9b046e393d46985aec364` ｜ 闸门 **16/16** ｜ `state.version` = **17** ｜ **内容提交链**（截至 `00b4bfc`）：`00b4bfc` 根 README 分离（tag `v7.20` 所指提交 `b00e788` 之后另有 **9 个提交**：协作机制 → 跨平台巡回适配 → 交接同步 → 铁律区建立 → 铁律首轮修订 + 审计补录 → A1 改写 → 环境文档补坑 → 再次交接同步 → 根 README 分离）｜ 远端 `origin/master` = `00b4bfc`（**最后一次已推送的内容提交**；tag `v7.9`/`v7.10`/`v7.11`/`v7.13`/`v7.18`/`v7.19`/`v7.20` 共 7 个全在远端）；⚠️ 其上另有**未推送的文档收口提交**——★**HEAD 与推送条数一律实测**（`git log -1 --oneline` / `git rev-list --count origin/master..HEAD`），**不在此冻结**（见 §交付惯例 2）。
...
- **v7.21c** = **「节」判定修正 + 灵感气泡按行内内容排组**（Holly 报告状态栏节名随位置漂移）——`structAt` 的 **label 改纯行首特征·行级**（`镜头N` 一律「分镜 镜头N」；无标记行悬空显示「**判断中**」不敲定），**region 保留段落延续**（label/region 职责分离，注释已写明；逗号豁免零影响）；**灵感气泡改按当前行内容排组**（候选组 = 本节相关组 ∪ 行内命中组；★**切词前剥离行首「镜头N」序号**——无内容语义的伪 token，防子串误命中「起手式」组；引擎零领域词表）。**新增 B6 守门断言**（v7.21 组 11 → 12；有齿证伪过）。旧 label「起手式」「叙事正文」从此不再出现（位置推导的产物）
- **v7.21b** = **项目总览面板低门槛改版**（Holly 实测首版判定「可读性极差」，要求**低门槛阅读**）——`320px` 窄窗 → **`min(880px,92vw)` 两栏 master-detail**（左栏按用途分组的**白话**清单〔写作 5 / 画布 2 / 项目与界面 3〕/ 右栏选中项详情 = **天然的技术细节位**）；完善度改**三档人话状态 + 彩色点**（去掉公式）；技术证据降级入右栏；全面板字号 **≥12px**。★**健康行改动态取数 + 句末「（交付时）」时效标记**（原为写死静态文案 → 将来闸门变红面板仍会宣称全通过；面板是 `file://` 离线单文件，无法运行时探测）。快照**同名覆盖**（同版本内迭代口径）；`verify_v721` 仅 **B2 判定式适配（只增不减）**，其余判定式与条数不变
...
`node dev/_qa/run-gate.mjs` → 期望 **16/16**：355975 B ｜ 等价性 PASS ｜ 83/83 ｜ F 18/18 ｜ G 16/16 ｜ v7.8 **54/54** ｜ 开发态 18/18 ｜ W 21/21 ｜ C 31/31 ｜ E 16/16 ｜ P 4/4（粘贴）｜ M 9/9（多项目）｜ V 4/4（迁移观感）｜ v7.19 10/10 ｜ v7.20 18/18 ｜ **v7.21 12/12**（标签 初/补 与分组 A×6 + 项目总览面板 B×5 + 灵感气泡按行排组 B6×1）
...
> **性质**：**惯例，不是铁律**（未入 `IRON_RULES.md`）——为项目健康发展而遵守；偏离时按协作规范纠正即可，无需上纲为事故。
> **来源**：2026-09-21 交接文档事实漂移事故提炼，详见 `dev/_qa/records/handover/交接_2026-09-21_接手探查与交接文档事实校正.md`。

1. **交付即同批同步交接状态**：凡有 **commit** 的交付，**必须在同一批**刷新 `HANDOVER.md` 的「本地 git / 远端」两行与抬头日期（涉及基线时一并刷新本文件的<当前基线>段）。
   - 《多agent异步协作协议》§五「交出方 3 步」的「更新 HANDOVER」应在**提交前**完成，使**交接闭环判据③**（`HANDOVER.md` 与实际一致）**在提交落地时即为真**，不留尾账。
   - **漂移根因不是"忘了更新"，而是提交本身会改变 HEAD**——凡把裸 hash 写成「HEAD / 最新 / 已同步」的，**写完即过期**。
2. **HEAD 必须写成「提交后仍成立」的形式**（推荐其一）：
   - **父提交 + 本提交性质**——例：`HEAD = 本轮文档交付提交（其父 = 00b4bfc）`；父 hash 在提交后不变，**`--amend` 也不失效**；
   - **本文件最后修改所在提交**——自校验：`git log -1 --format='%h %s' -- HANDOVER.md`。
   - ❌ **禁**：`HEAD = 00b4bfc` / `HEAD = 本轮最新` 这类**裸 hash 或相对表述**（下次提交即失真）。
3. **数量类陈述先数物证再写**（同源教训）：闸门次数、复发率、diff 行数、条数等**必须由磁盘物证（日志 / 命令输出）逐份计数后落笔**，不得凭记忆汇总；写完把计数命令一并留档。
   - 实证：2026-09-21 主理人一度把「实跑 4 次闸门」误记为 3 次（自查后 amend 修正）。
4. **判据③的口径**：「与实际一致」＝ **HEAD / 远端 / 版本 / 指纹 / 在途工作 / 待裁决项** 六类字段均与实测一致；**不要求**每次提交都改 `HANDOVER.md`——纯代码提交若不动这六类字段，只需确认仍成立。
