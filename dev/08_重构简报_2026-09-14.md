...
> 作者：高见远（Architect）｜ 日期：2026-09-14（**v7.8 增量更新**）｜ 状态：**待 Holly 拍板**
...
| # | 待确认项 | 我的倾向 | 理由 |
|---|---|---|---|
| **Q1** | **重构深度**：主推（真模块化）还是备选（只止血）？ | 主推（**v7.8 后更坚定**） | 债在复利：v7.8 令 E1/E4/E5/E6 **全部恶化**（+19 var / +3 Escape 点 / +2 全局监听 / 严格覆盖 1/19）；且新团队**已证明"按职责切"可行**（E11）→ 主推路径无技术障碍 |
| **Q2** | **构建期是否允许 npm 依赖**（如 esbuild/rollup）？ | **不允许** | "任意机器复现"是产品红线；零依赖 plain-node 拼接对 ~3.3k 行足够 |
| **Q3** | 是否**确认维持"改义后的闸门"**（=构建可复现/源码↔交付物一致性），并**明确其全程有效**（不再 P2 退役）？ | **确认** | 改义后该闸门证明"交付单文件确由这份 src 构建"，**比原闸门更有价值**且迁移期恒成立；A/B 行为对照另用 v7.7 基线 |
| **Q4** | P2 的**切分粒度**（模块数/文件数）是否需先出细则？ | 需要 | 建议以 **v7.8 的 `51/52/53` 为首批样板**反向定 `manifest.mjs` 草案，再评估 |
| **Q5** | **★产品体积上限口径**：定多少？（128K→151.6K→**209.4KB@v7.8**） | **硬上限 256KB（262144 B）**；闸门预算暂用工程师的 **229376 B（224KB）** | v7.8 的 +38% 是**实打实新功能**（候选引擎 + 配置窗 + 内置片段表），**非口径问题**；对"双击即用"**无实质影响**（214KB 本地文件双击瞬开、无网络）；但趋势需护栏。224KB 作**回归护栏临时值**可接受，须在 README 注明"临时、非产品政策"，并**另设产品口径** |
| **Q6** | 等价性闸门改义是否**写进 README 与交接**并**修正原稿旧表述**？ | 确认 | 原 §3 资产表"仅 P0–P1 有效"已按 Q3 修正；交接须与 README「构建可复现 ✅」一致 |
| **Q7** | **★内置「风格包」逐字烧进产品代码**是否可接受（`52-complete.js:20-27`）？ | **建议外置/降为"示例数据"** | 那是**项目级私有写作资产**（542 字符）；硬编码进通用代码 → 对外分发即**随码发布他人资产**，且占体积；宜入配置或标"示例" |
| **Q8** | v7.8 的 `51/52/53`：**冻结现状**（走备选）还是**纳入 P2 重切**？ | **纳入 P2** | 它们是**已按职责命名**的现成单元，纳入成本最低、收益最高（首迁样板） |
